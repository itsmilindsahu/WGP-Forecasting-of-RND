"""
Runs Day 1 -> Day 4 end-to-end using synthetic data (see README for why real
Deribit/yfinance fetchers are separate, network-requiring scripts).
"""
import os
import sys
import numpy as np
...
os.makedirs(os.path.join(ROOT, "day1_data_collection", "data"), exist_ok=True)
os.makedirs(os.path.join(ROOT, "day2_rnd_extraction", "data"), exist_ok=True)
os.makedirs(os.path.join(ROOT, "day4_gp_model", "results"), exist_ok=True)

sys.path.append(os.path.join(ROOT, "day5_baseline"))
sys.path.append(os.path.join(ROOT, "day6_evaluation"))

from simulate_data import simulate_chain  # noqa: E402
from fit_iv_curve import smooth_all_days  # noqa: E402
from breeden_litzenberger import extract_all_days  # noqa: E402
from wasserstein_kernel import density_to_quantile, build_kernel_from_densities  # noqa: E402
...
os.makedirs(os.path.join(ROOT, "day5_baseline", "results"), exist_ok=True)
os.makedirs(os.path.join(ROOT, "day6_evaluation", "results"), exist_ok=True)


def main():
    print("### DAY 1: simulate + clean option chain, fit smoothed IV curve ###")
    chain = simulate_chain(n_days=40)
    ...
    print(f"  {chain['date'].nunique()} days, {len(chain)} raw chain rows -> smoothed IV curves written.\n")
    ...
    print(f"  Extracted RNDs for {rnd['date'].nunique()} days.")
    print(
        "  Martingale check (extracted mean vs forward, tol=1%): "
        f"{len(mtg_diagnostics) - n_fail}/{len(mtg_diagnostics)} days pass, "
        f"max rel_error = {mtg_diagnostics['rel_error'].max():.4%} "
        "(replaces the vacuous 'integrates to 1.0' check, which the "
        "renormalization step guarantees will always pass)\n"
    )

    print("### DAY 3: Wasserstein kernel construction + synthetic PSD validation ###")
    ...
    print(f"  Built {K.shape[0]}x{K.shape[0]} kernel matrix on real (simulated) RND sequence.")
    print(f"  min eigenvalue = {eigvals.min():.3e} (PSD check: {'OK' if eigvals.min() > -1e-6 else 'FAIL'})")
    print("  (see day3_wasserstein_kernel/test_synthetic.py for the dedicated closed-form + PSD unit test)\n")
    ...
    print("### DAY 5b: persistence benchmark (the model to beat, per Sven's review) ###")
    ...
    print("### DAY 6: evaluate GP vs baseline vs persistence (coverage, NLPD, W2 forecast error) ###")
    ...
    print("\nAll outputs written. Days 1-7 pipeline complete.")
    print("Results memo: day7_writeup/results_memo.md")
    print("Interactive demo: day7_writeup/docs/index.html")
